...
dataPath = '/home/fiore/MTP/data/'; % where to save logged data locally
...
if logFlag
    ...
    if fid == -1
        ...
        fprintf(fid, 'Propeller Tilt:       %.2f deg\n', rad2deg(config.uavParams.propTilt));
...
if ~experimentFlag 
    result = dynamixel.connect('/tmp/pty-dynamixel');
    checkResult(result.status, 'dynamixel.connect', result.exception);

    result = dynamixel.set_position(num2cell(config.uavParams.propTilt*ones(8,1)));
    checkResult(result.status, 'dynamixel.set_position', result.exception);
end
